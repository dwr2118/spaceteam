...
void loop() {
  ...
  if (millis() - lastUpdatedTime >= 10000) {
    Serial.println("----------------");
    Serial.println("generating command");
    Serial.println("----------------");
    //cmd1 = genCommand();
    //cmd2 = genCommand();
    lastUpdatedTime = millis();
    //clear();
    // ********
    // FIGURE OUT HOW TO BLACK SCREEN HERE TO AVOID DRAWING OVER TEXT

    //display.fillRect(0, 0, displayWidth, displayHeight, TFT_BLACK);
    tft.fillScreen(TFT_BLACK);
    drawControls();
    //scheduleCmd1Send = true;
    //scheduleCmd2Send = true;
